    ...
    **Content:** 
    ```json 
    {
        "data": {
            ...
            "credit": "0.0000",
            "count_users": 1
            "phone_manager": "+79999999999"
            "email_manager": "ivan@ivan.ru"
            "name_manager": "Ivan"
        }
    }
    ```
...
* **Тело запроса**
  ...
  { 
    ...
    "desired_date": "дата отгрузки в формате Y-m-d"
  }
  ```
...
**Фильтрация товара**
----
  Осуществляет фильтрацию товаров по указанным свойствам. Возможна фильтрация по нескольким свойства и по нескольким 
значениям одного свойства, разделителем в таком случае будет ; То есть например, style1=пиво;водка
...
----
  ...
  Допустимо также использовать предопределенные временные параметры. Доступны следующие параметры:
  day, week, month, half-year, year. Например, для того, чтобы получить все движения по договорам со вчерашнего
  дня, необходимо обратиться по адресу:
    
    /api/dogovors?start_date=week
...
* **Тело запроса**
  ...
  ```json
  {
      "address": "id адреса для самовывоза",
      "fio": "ФИО пользователя(надо ли?)",
      "phoneNumber": "номер пользователя",
      "desired_date": "ожидаемая дата доставки"
  }
  ```
    ...
    ```json 
        {
            "data": [
                {
                    "id": 1,
                    "address": "Москва, Новомещерский проезд, д. 9, стр. 1, терминал "Солнечный",
...
**Новость**
----
Возвращает одну из новостей по id. Если добавить GET параметр count=число, то тогда будет выведены последние 
...
**Создать новость**
----
Создается новость. Доступ к роуту осуществляется только ключ news-key в http-header, значение которого можно уточнить.
...
**Изменить новость**
----
Изменяется новость. Доступ к роуту осуществляется только ключ через news-key в http-header, значение которого можно уточнить.
...
**Удалить новость**
----
Удаляет одну из новостей по id. Доступ к роуту осуществляется только через news-key в http-header, значение которого можно уточнить.
...
**Информация о задолженности организации**
----
Возвращает информацию о движении по договорам организации
    ...
    ```json 
      {
        "data": [
            {
                "dogovor_id": 19,
                "dogovor_name": "Договор поставки товара №СГ44/16 от 06.07.2016",
                "credits": [
                    {
                        "id": 8,
                        "dogovor_id": 19,
                        "document": "Реализация товаров и услуг СГ000016462 от 26.03.2021 12:01:31",
                        "pay_date": "2021-04-09 09:01:31",
                        "period": "2021-03-26 12:01:31",
                        "external_id": "1eadce80-8d63-11eb-bbb2-000c29897a28",
                        "dolg": "66742.62",
                        "created_at": "2021-03-29 07:24:22",
                        "updated_at": "2021-03-29 07:24:22"
                    }
                ],
                "credit_sum": 66742.62
            }
